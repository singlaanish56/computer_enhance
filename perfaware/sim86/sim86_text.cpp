...
static void PrintInstruction(instruction Instruction, FILE *Dest)
{
 ...
 static std::unordered_map<std::string, std::pair<int, u16>> um = {
     {"al", {0, 0b0000000011111111} }, {"ah", {0, 0b1111111100000000} }, {"ax", {0, 0b1111111111111111} },
     {"bl", {1, 0b0000000011111111} }, {"bh", {1, 0b1111111100000000} }, {"bx", {1, 0b1111111111111111} },
     {"cl", {2, 0b0000000011111111} }, {"ch", {2, 0b1111111100000000} }, {"cx", {2, 0b1111111111111111} },
     {"dl", {3, 0b0000000011111111} }, {"dh", {3, 0b1111111100000000} }, {"dx", {3, 0b1111111111111111} },
     {"sp", {4, 0b1111111111111111} }, {"bp", {5, 0b1111111111111111} }, {"si", {6, 0b1111111111111111} }, 
     {"di", {7, 0b1111111111111111} },
 };
...
static void SimulateInstruction(instruction Instruction, u16* Registers_Storage, FILE *Dest)
{
    ...
    for(u32 operandIndex=1; operandIndex<ArrayCount(Instruction.Operands); operandIndex++)
    {
        ...
        if (Operand.Type != Operand_None){
            switch(Operand.Type)
            {
                ...
                case Operand_Register:{
                    ...
                    value  = Registers_Storage[um[reg_name].first] & um[reg_name].second;
                    break;
                }
                case Operand_Immediate:{
        ...
        auto dest_reg_name = GetRegName(dest);
        auto dest_old_value = Registers_Storage[um[dest_reg_name].first];
        Registers_Storage[um[dest_reg_name].first] = value;

        fprintf(Dest, " ; %s:0x%d->0x%d", dest_reg_name,dest_old_value, value);
